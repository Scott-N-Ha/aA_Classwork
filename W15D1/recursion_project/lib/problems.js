...
function pow(base, exponent) {
  ...
  if (exponent > 0) return base * pow(base, exponent-1);
...
function fileFinder(directories, targetFile) {
  ...
  Object.keys(directories).forEach(file => {
    if (file[0] === '/') {
      let result = fileFinder(directories[file], targetFile);
      if (result) found = true;
    } else if (file === targetFile) found = true;
  });
...
function pathFinder(directories, targetFile) {
  ...
  Object.keys(directories).forEach(file => {
    if (file[0] === '/') {
      let result = pathFinder(directories[file], targetFile);
      if (result !== null) found = file + result;
    } else if (file === targetFile) found = '/' + file;
  });
