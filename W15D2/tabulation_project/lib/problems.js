...
function stepper(nums) {
  const steps = Array(nums.length).fill(false);
  steps[0] = true;

  for( let i = 0; i < nums.length && steps[i]; i++){
    let numSteps = nums[i];

    if (!steps[i+numSteps]){
      while(numSteps > 0){
        let stepTaken = i + numSteps;
  
        if (stepTaken < nums.length) steps[i+numSteps] = true;
...
maxNonAdjacentSum([4, 1, 1, 10, 3, 2])         // => 10, because 4 + 6 
...
function maxNonAdjacentSum(nums) {
  ...
  for (let i = 2; i < nums.length; i++){
    ...
    for (let j = i-2; j >= 0; j--){
      max = Math.max(nums[i] + sums[j], nums[i], max);
    }
...
function minChange(coins, amount) {
  const counts = Array(amount+1).fill(0);

  for(let i = 1; i <= amount; i++){
    if (coins.find(coin => coin === i)) counts[i] = 1;
    else {
      ...
      coins.forEach(coin => {
        let diff = i - coin;
        if (diff > 0){
          min = Math.min(min, counts[diff] + 1);
        }
      });
